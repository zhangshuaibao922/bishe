...
//    }
    ...
    public Teacher update(Teacher teacher){
        //1. 根据条件动态更新
        LambdaUpdateChainWrapper<Teacher> chainWrapper = queryConditions(teacher);
        //2. 设置主键，并更新
        chainWrapper.set(Teacher::getId, teacher.getId());
        boolean ret = chainWrapper.update();
        //3. 更新成功了，查询最最对象返回
        if(ret){
            return queryByTeacherId(teacher.getTeacherId());
        }else{
            return teacher;
        }
    }
    ...
    public boolean deleteByTeacherId(String teacherId){
        LambdaQueryWrapper<Teacher> teacherLambdaQueryWrapper = new LambdaQueryWrapper<>();
        int total = teacherMapper.deleteById(teacherLambdaQueryWrapper.eq(Teacher::getTeacherId,teacherId));
        return total > 0;
    }
